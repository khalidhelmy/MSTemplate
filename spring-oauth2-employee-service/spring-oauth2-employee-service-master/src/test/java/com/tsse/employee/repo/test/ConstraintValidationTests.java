...
public class ConstraintValidationTests extends EmployeeRepositoryIntegrationTest {
    ...
    @Test
	public void addInvalidEmployeeFirstName() {
		Employee emp = constructEmployee(null, 18, null, "John"); // First name shouldnt be null
		Set<ConstraintViolation<Employee>> violations = validator.validate(emp);
        assertFalse(violations.isEmpty());
    ...
    @Test
	public void addInvalidEmployeeLastName() {
		Employee emp = constructEmployee(null, 18, "Alex", null); // last name shouldnt be null
		Set<ConstraintViolation<Employee>> violations = validator.validate(emp);
        assertFalse(violations.isEmpty());
